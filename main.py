...
if __name__ == "__main__":
    ...
    if debug_mode:
        logger.add(
            "log/debug_{time}.log", rotation="1 day", retention="14 days", level="DEBUG"
        )
    else:
        logger.add(
    ...
    assert base_url, "Please set BASE_URL in config.py"
    ...
    if use_tg_notification:
        ...
        assert bot_token and user_id, "Please set BOT_TOKEN and USER_ID in config.py"
    ...
    assert subscribe_url, "Please set SUBSCRIBE_URL in config.py"
    download_path = getattr(config, "DOWNLOAD_PATH", None)
    assert download_path, "Please set DOWNLOAD_PATH in config.py"
    manager = RssManager(
        subscribe_url,
    ...
    assert user_name, "Please set USER_NAME in config.py"
    password = getattr(config, "PASSWORD", None)
    assert password, "Please set PASSWORD in config.py"
