...
from openlist_ani.assistant.builtin_skills.support.anime_recommend_confidence import (
    compute_confidence,
    format_confidence_report,
)
from openlist_ani.assistant.builtin_skills.support.anime_recommend_scoring import (
...
def _resolve_memory_dir() -> Path:
    ...
    return assistant_data_dir() / "memory"
...
async def _fetch_and_score_calendar(
    ...
    # Build calendar text (same format as the Bangumi weekly_calendar script)
...
async def run(**kwargs) -> str:
    ...
    This eliminates separate calendar and score-candidate script calls.
    """
    ...
    if not token:
        return (
            ...
            "1. Run the Bangumi weekly_calendar script\n"
            "2. Run score_candidates.py with the output\n"
    ...
    if not has_changes and existing_profile:
        ...
        parts.append(
            ...
            f"{analysis_data}\n\n"
            "---\n"
            "Offer to save this profile. After a later explicit user "
            "confirmation, run `scripts/save_profile.py` with the exact "
            "profile text and `confirmed=true`. Recommendations may continue "
            "without saving.\n\n"
            "**Rules:**\n"
    ...
    if calendar_scored:
        ...
        parts.append(
            "\n---\n\nCalendar fetch failed. Run the Bangumi weekly_calendar script manually."
        )
