...
class ChatGPT:
    ...
    async def analyse_resource_name(self, resource_name: str):
        ...
        后续我将会给你提供一个番剧的资源名字，请你根据番剧名字，提取出字幕组名称，类型为string，存储在fansub字段中；番剧的季度，类型为int，存储在season字段中，如果是特别篇/总集篇则季度设置为0，没有特别标注的默认为第1季；番剧的集数，类型为int，存储在episode字段中；番剧的清晰度，类型为string，以"xp"的格式存储在quality字段中，例如"1920x1080"请重命名为"1080p"。最后以YAML的格式返回。YAML具体格式如下：
        ```yaml
        fansub:
        season:
        episode:
